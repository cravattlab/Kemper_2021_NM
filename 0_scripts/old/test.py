...
def aggregate_stats(group_dataset, inputs):
    ...
    num_cv = numerator + '_CV'
    denom_cv = denominator + '_CV'
    num_median = numerator + '_median'
    denom_median = denominator + '_median'
    num_rep_cols = [col for col in rep_cols if numerator in col]
    denom_rep_cols = [col for col in rep_cols if denominator in col]
    df1['num_min'] = df1[num_rep_cols].min(axis = 1)
    df1['num_max'] = df1[num_rep_cols].max(axis = 1)
    cond1 = ((df1['num_min'] > 2) & (df1[denom_cv] > 0.5))
    cond2 = ((df1['num_max'] < 0.5) & (df1[denom_cv] > 0.5))
    df1['big_diff_denom'] = (cond1)|(cond2)
    return df1

def qc_data(group_dataset, inputs):
    df = aggregate_stats(group_dataset, inputs)
    unfiltered_df = df.copy()
    # If the CV is high but all the individual reps are changing at least 2x fold the QC won't catch for denom so need to separate
    big_diff_df = df.loc[df['big_diff_denom'] == True]
    qc_df = df.loc[df['big_diff_denom'] == False]
    rep_cols = sorted([col for col in df.columns if '_rep' in col])
    #Get rid of all cols with experiment (Exp456) attached, I think this is so we can stitch back together without column duplicates
    keep_cols = [col for col in big_diff_df.columns if '_' + group_dataset not in col]
    big_diff_df = big_diff_df[keep_cols]
    short_df = qc_df[protein_cols + rep_cols]
    ...
    numerator = inputs.num.item()
    denominator = inputs.denom.item()
    group_names = [numerator, denominator]
    for group in sorted(set(group_names)):
        print(group)
        col = [col for col in qc_df.columns if group in col]
        test_df = qc_df[protein_cols + col]
        ...
        group_reps = [col for col in short_df.columns if '_rep_' in col]
        #if the CV is high but there is a big enough difference between all the channels then keep it
        big_diff_df[group + '_category'] = 'Keep, CV high but all num values higher'
    short_df.reset_index(inplace = True)
    df1 = short_df.append(big_diff_df, sort = True)
    df1 = df1.dropna(axis=1, how='all')
    #calculate ratio median
    df1.loc[df1[denominator+ '_median'].isnull(), numerator + '_median'] = np.nan 
...
#for a dataset: perform functions defined above
    ...
    if a == 'all':
        for group_dataset in expt_table.index:
            ...
            df = qc_data(group_dataset, inputs)
            df.to_csv('{}/0_scripts/replicates/{}_peptide_{}.csv'.format(dir, date,group_dataset), index = False,float_format='%.2f')
