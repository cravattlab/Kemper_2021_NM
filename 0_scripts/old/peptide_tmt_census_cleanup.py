...
def clean_census_out_6plex(dataset_name, inputs):
...
def clean_census_out_10plex(dataset_name, inputs):
    """Get census_out from IP2 and get it into working form, for 10-plex, for 6-plex see 201908091_Exp357_TMT_processing.py"""
    #census_out.txt from IP2
    #Had to re-save .txt as tab-delimited otherwise wouldn't read... will figure this out later
...
def read_census(dataset_name, no_missed_sites, inputs):
    ...
    not_assigned = df.loc[df.tryptic_position == -1]
    if len(not_assigned) > 0:
        print('Error with assigning')
    else:
        print('Assigning is okay')
    df['residue'] = df.tryptic_position + df.pos
    df['gene_res'] = df['gene_y'] + '_' + df['residue'].astype(str)
    ...
    df = df.drop(['pos', 'gene_x', 'gene_y', 'residue', 'tryptic_position','description_x',  'sequence_x', 'sequence_y', 'no_mods'], axis = 1) #
