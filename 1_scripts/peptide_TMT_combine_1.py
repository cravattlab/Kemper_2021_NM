    ...
    if a == 'all':
        for exp_group in list(set(expt_table.group)):
            ...
            for group_dataset in expt_table_subset.index:
                ...
                if phosphopep == 'yes':
                    df = pd.read_csv('{}/0_scripts/replicates/{}_phospho_peptide_{}.csv'.format(dir, date,group_dataset))
                else:
            ...
            if phosphopep == 'yes':
                df1.to_csv('{}/1_scripts/combined/{}_phospho_combined_{}.csv'.format(dir, date,group_name), index = False)
            else:
