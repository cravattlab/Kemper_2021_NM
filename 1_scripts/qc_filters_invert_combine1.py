...
def qc_filtering(row):
    ...
    reps = [col for col in row.index if '_median_TMT' in col]
    ...
    if np.isnan(row[old_cv]) == True:
        ...
        if row[old_cv] <= 0.6:
            ...
            if row[combined_min] >= 1.5:
                ...
                if row[old_cv] <= 0.6:
                    return pd.Series([row[old_median], row[old_cv],row[no_cols],'Keep, more than 4 reps and above 0.6'], index=headers)
                elif row[old_cv] > 0.6:
                    if percent_passing >= 0.8:
                        return pd.Series([row[old_median], row[old_cv],row[no_cols],'Keep, high CV but 80 percent of values 1.6'], index=headers)
                    else:
        ...
        new_std = np.nanstd(new_reps, ddof=1)
        new_mean = np.nanmean(new_reps)
        new_cv = new_std/new_mean
        new_min = np.nanmin(new_reps)
        if new_cv <= 0.6:
            ...
            if new_min >= 1.5:
                ...
                percent_passing = no_passing/total_no
                if new_cv <= 0.6:
                    return pd.Series([row[old_median], new_cv,row[no_cols],'Keep, more than 4 reps and above 0.6'], index=headers)
                elif new_cv > 0.6:
                    if percent_passing >= 0.8:
                        return pd.Series([row[old_median], new_cv, row[no_cols], 'Keep, high CV but 80 percent of values 1.6'], index=headers)
                    else:
            ...
            # print('Error???')
            return pd.Series([row[old_median], new_cv, row[no_cols],'Could not categorize'], index=headers)
    else:
        print('Error1')
        return pd.Series([row[old_median], new_cv, row[no_cols],'Could not categorize'], index=headers)
